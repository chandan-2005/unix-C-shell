...
static int count_non_empty_lines(const char *filename) {
    FILE *fp = fopen(filename, "r");
    if (!fp) return 0;
    int count = 0;
    char *l = NULL; size_t sz = 0;
    while (getline(&l, &sz, fp) != -1) {
        if (is_non_empty(l)) count++;
    }
    free(l);
    fclose(fp);
    return count;
}
static void print_forward(FILE *fp, int flag_n, int *line_num) {
    char *line = NULL;
    size_t len = 0;
...
static void print_reverse_chunked(int fd, int flag_n, int *line_num) {
    ...
    if (read(fd, &last_byte, 1) == 1 && last_byte == '\n') {
        filesize -= 1;
    }
    if (filesize <= 0) return; 
    ...
    if (line_end > 0) {
        ...
        if (flag_n && is_non_empty(line_buf)) printf("%d ", (*line_num)--);
        printf("%s", line_buf);
        if (line_buf[len-1] != '\n') printf("\n");
        free(line_buf);
    }
...
void run_peek(Token *tokens) {
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (flag_r) {
            ...
            print_reverse_chunked(fd, flag_n, &line_num);
            close(fd);
        } else {
            FILE *fp = fopen(filename, "r");
            if (!fp) { perror("peek"); continue; }
            print_forward(fp, flag_n, &line_num);
            fclose(fp);
        }
    }
}